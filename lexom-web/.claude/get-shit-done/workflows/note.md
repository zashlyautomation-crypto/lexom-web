...
- **Global scope**: `D:/lexom/lexom-web/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/lexom/lexom-web/.claude/notes/`)
...
2. Glob `D:/lexom/lexom-web/.claude/notes/*.md` (if directory exists) — global notes
...
Global (D:/lexom/lexom-web/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/lexom/lexom-web/.claude/notes/` — works in any directory
